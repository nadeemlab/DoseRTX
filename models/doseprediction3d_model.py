...
class DosePrediction3DModel(BaseModel):
    ...
    def backward_G(self):
        ...
        # self.loss_G_DVH = 10*self.criterionDVH(self.fake_Dose, self.target_hist, self.target_bins, self.real_CT[:, 2:,...])#Changed(Gourav)
        self.loss_G_MOMENT = 0.001*self.criterionMoment(self.fake_Dose, self.real_CT[:, 2:, ...], self.real_Dose)  # Changed for  beam(Gourav)
