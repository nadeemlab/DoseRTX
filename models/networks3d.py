...
class MomentLoss(nn.Module):
    ...
    def __call__(self, predicted_dose, oar, dose):
        ...
        return self.loss(oarPredMoment, oarRealMoment)#self.loss(oarPredMoment, oarRealMoment)#self.loss(oarPredMoment, torch.zeros_like(oarPredMoment))#torch.sum(oarPredMoment)#, torch.zeros_like(oarPredMoment))