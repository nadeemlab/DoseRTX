...
def transform_ct_dose_pair2D(ct_arr, dose_arr, oar_arr, ptv_arr, apply_transform=False):
  ...
  if apply_transform is True:
      if random.random() < 0.5:  # 50 % chance of applying random transformation
        ...
        dose_pil = TF.affine(dose_pil, angle=angle, translate=(w,h), scale=scale, shear=(x,y), resample=Image.BILINEAR)
        oar_pil = TF.affine(oar_pil, angle=angle, translate=(w, h), scale=scale, shear=(x, y), resample=Image.NEAREST)
        ptv_pil = TF.affine(ptv_pil, angle=angle, translate=(w, h), scale=scale, shear=(x, y), resample=Image.NEAREST)
...
def transform_3d_data(ct_arr, dose_arr, oar_arr, ptv_arr, hist_arr, bins_arr, transform=False):
