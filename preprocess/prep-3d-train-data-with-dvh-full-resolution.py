...
def crop_resize_img(img, start, end, is_mask=False):
	...
	print('After crop setting {}'.format(img_cropped.shape))
	img_cropped = np.moveaxis(img_cropped, 0, -1)  # Slices last

	order = 0
	if is_mask is False:
		order = 1
	img_resized = resize(img_cropped, (256, 256, 256), order=order, preserve_range=True, anti_aliasing=False).astype(np.float32)
...
def process_case(in_dir, out_dir, case):
	...
	print('Before crop setting {} {} {}'.format(ct.shape, oar.shape, dose.shape))
	start, end = get_crop_settings(oar_copy)
	print(end[0]+1 - start[0])
	ct = crop_resize_img(ct, start, end, is_mask=False)
	oar = crop_resize_img(oar, start, end, is_mask=True)
...
in_dir = '/nadeem_lab/Gourav/lung-manual-dicomextracted'
...
out_dir = '/nadeem_lab/Gourav/datasets/msk-manual-3d-imrt-sparse-seperate-ptv-full-resolution/test'
